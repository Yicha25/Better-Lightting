...
(function() {
    ...
    const SSRT_SHADER = `
        ...
        vec3 getSmoothNormal(vec2 uv, float depth, vec2 sz) {
            float rad = smoothNormals ? 8.0 : 1.0;
        ...
        void main() {
            ...
            if (!isEnabled || depth >= 1.0) { gl_FragColor = texture2D(colorTexture, v_textureCoordinates); return; }
            ...
            vec3 rayPos = pos + norm * (0.4 + jitter * 0.1);
            ...
            for (int i = 0; i < 45; i++) {
                ...
                hitPos += reflectDir * currentStepSize;
                currentStepSize *= 1.05; 

                if (length(hitPos - pos) > maxSearchDistance) break;

                vec4 proj = czm_projection * vec4(hitPos, 1.0);
                ...
                if (sceneD >= 1.0) continue;

                vec3 sceneP = getViewPos(hitUV, sceneD);
                float zDiff = sceneP.z - hitPos.z;

                if (zDiff > 0.0 && zDiff < (1.5 + length(hitPos - pos) * 0.02)) {
                    hit = true;
                    break;
                }
            }
            ...
            if (hit) {
                ...
                float distFade = 1.0 - clamp(length(hitPos - pos) / maxSearchDistance, 0.0, 1.0);
                
                float angleFade = smoothstep(0.0, 0.15, dot(norm, -viewDir));

                vec4 refColor = texture2D(colorTexture, hitUV);
                
                gl_FragColor = mix(baseColor, refColor, strength * fresnel * edgeFade * distFade * angleFade);
            } else {
    ...
    class SSRTManager {
        constructor() {
            this.state = { isEnabled: true, smoothNormals: true, strength: 0.7, maxDist: 3000 };
            this.lastModel = null;
            this.stage = null;
        }
        ...
        ui() {
            ...
            target.insertAdjacentHTML('beforeend', `
                <div id="ssrt-ui" style="border-top: 1px solid #555; margin-top: 10px; padding-top: 10px;">
                    <h5 style="color: #00d4ff; font-weight: bold;">Better Lightting</h5>
                    ...
                    <div class="geofs-option"><span>Smooth Normals</span><label class="mdl-switch mdl-js-switch" for="s2"><input id="s2" type="checkbox" class="mdl-switch__input" checked></label></div>
                    <div class="geofs-option"><span>Strength</span><input id="sl1" type="range" class="mdl-slider" min="0" max="1" step="0.01" value="0.7"></div>
                </div>
            `);
            
            document.getElementById('s1').onchange = (e) => this.state.isEnabled = e.target.checked;
            document.getElementById('s2').onchange = (e) => this.state.smoothNormals = e.target.checked;
            document.getElementById('sl1').oninput = (e) => this.state.strength = parseFloat(e.target.value);
            if (window.componentHandler) window.componentHandler.upgradeElements(document.getElementById('ssrt-ui'));
        }
    }

    if (window.geofs && window.Cesium) new SSRTManager().init();
})();
